...
{
	private readonly SecurityAdvisor securityAdvisor = new();
	...
	private void PopulateControls()
	{
		...
		var (userExists, passwordIsDefault, adminUserId) = securityAdvisor.DefaultAdmin();
		if (userExists)
		{
			var manageUserLink = $"<a href=\"{SiteRoot}/Admin/ManageUsers.aspx?userid={adminUserId}\" class=\"btn btn-sm btn-default\">{Resource.ManageUsersTitleLabel}</a>";
			litDefaultAdminAccountResults.Text = $"<div class=\"alert alert-danger\"><strong>{Resource.Attention}</strong> {(passwordIsDefault ? Resource.SecurityAdvisorDefaultAdminAccountExistsAndPasswordIsDefault : Resource.SecurityAdvisorDefaultAdminAccountExists)} {manageUserLink}</div>";
		}
		else
		{
		...
		else
		{
			litMachineKeyResults.Text = $@"<div class=""alert alert-danger""><strong>{Resource.Attention}</strong> {Resource.SecurityAdvisorMachineKeyWrong}</div>
					...
					<div>{Resource.CustomMachineKeyInstructions}</div>
	...
	{
		AddClassToBody("administration securityadvisor");
	...
	{
		const string sslCheckUrl = "https://howsmyssl.com:443/a/check";
		try
		{
			var request = WebRequest.CreateHttp(new Uri(sslCheckUrl));
		...
		{
			log.Info($"Security Advisor could not connect to \"{sslCheckUrl}\". Error was {ex}");
		}
	}
