	...
	protected override void RenderContents(HtmlTextWriter writer)
	{
		...
		mojoMenuItem getMenuItemFromPageSettings(PageSettings pageSettings)
		{
			...
			return new mojoMenuItem
			{
				...
				Description = Global.SkinConfig.Menu.UseDescriptions ? pageSettings.MenuDescription : string.Empty,
				ShowDescription = Global.SkinConfig.Menu.UseDescriptions && !string.IsNullOrWhiteSpace(pageSettings.MenuDescription),
				Image = Global.SkinConfig.Menu.UseImages ? pageSettings.MenuImage : string.Empty,
				ShowImage = Global.SkinConfig.Menu.UseImages && !string.IsNullOrWhiteSpace(pageSettings.MenuImage),
		...
		void renderDefaultView(string error = "")
		{
			...
			{
				log.Error($"chosen layout ({View}) for Menu control was not found in skin {SiteUtils.GetSkinBaseUrl(true, Page)}. perhaps it is in a different skin. Error was: {error}");
			}

			renderString = RazorBridge.RenderPartialToString("Menu", model, "Shared");
		}
