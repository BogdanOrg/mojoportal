...
public partial class EditHtml : NonCmsBasePage
{
	...
	private void PopulateControls()
	{
		...
		if ((workInProgress != null) && (ViewMode == PageViewMode.WorkInProgress))
		{
			...
			lnkCompareDraft.NavigateUrl = $"{SiteRoot}/HtmlCompare.aspx?pageid={pageId.ToString(CultureInfo.InvariantCulture)}&mid={moduleId.ToString(CultureInfo.InvariantCulture)}&d={workInProgress.Guid}";
		...
		{
			var rHistory = new ContentHistory(restoreGuid);
	...
	private void BindHistory()
	{
		...
		if (module is not null && html.ModuleGuid == Guid.Empty)
		{
			html.ModuleGuid = module.ModuleGuid;
		}
	...
	{
		var btnDelete = (Button)e.Row.FindControl("btnDelete");
		btnDelete?.Attributes.Add("OnClick", $"return confirm('{Resource.DeleteHistoryItemWarning}');");
	}
	...
	void btnRestoreFromGreyBox_Click(object sender, ImageClickEventArgs e)
	{
		...
		var h = new Guid(hdnHxToRestore.Value);
	...
	private void SetupScript()
	{
		...
		script.Append("\n<script type='text/javascript' data-loader='HtmlEdit.aspx'>");
	...
	private void LoadSettings()
	{
		...
		isAdmin = WebUser.IsAdminOrContentAdmin || SiteUtils.UserIsSiteEditor();
