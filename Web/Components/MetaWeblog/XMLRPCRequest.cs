...
{
    private static readonly ILog log = LogManager.GetLogger(typeof(XMLRPCRequest));
    ...
    private static bool GetPublish(XmlNode node)
    {
        ...
        string result = statusNode.LastChild.InnerText;
    ...
    private static string GetCategory(XmlNode node)
    {
        ...
        return categoryNode.LastChild.InnerText;
    }
    ...
    private static string GetBlogId(XmlNode node)
    {

        return node.InnerText;
    }

    private static bool GetIsPublished(XmlNode node)
    {
        XmlNode status = node.SelectSingleNode("value/struct/member[name='post_status']");
        if ((status != null) &&(status.LastChild.InnerText == "publish")) { return true; }
	...
	XmlDocument request; 
        try
        {
			...
			request = Core.Helpers.XmlHelper.GetXmlDocumentFromString(xml);
        }
        catch (Exception ex)
        {
        ...
        switch (this.MethodName)
        {
            ...
            case "metaWeblog.newPost":
                this.BlogID = GetBlogId(this.inputParams[0]);
                ...
                {
                    this.Publish = GetIsPublished(this.inputParams[3]);
                }
            ...
            case "wp.newCategory":
                ...
                this.UserName = this.inputParams[1].InnerText;
                this.Password = this.inputParams[2].InnerText;
                this.Category = GetCategory(this.inputParams[3]);
                break;
            ...
            case "wp.getUsersBlogs":
                this.UserName = this.inputParams[0].InnerText;
                this.Password = this.inputParams[1].InnerText;
                break;
            ...
            case "wp.editPage":
                ...
                if (this.inputParams.Count > 5)
                {
                    this.Publish = this.inputParams[4].InnerText != "0" && this.inputParams[4].InnerText != "false";
                }
                else
                {
                    this.Publish = GetPublish(this.inputParams[4]);
                }
