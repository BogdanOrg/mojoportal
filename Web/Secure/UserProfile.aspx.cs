...
public partial class UserProfile : NonCmsBasePage
{
	...
	private void PopulateControls()
	{
		...
		if (siteUser != null)
		{
			...
			{
				var wl = WindowsLiveHelper.GetWindowsLiveLogin();
				var wlm = new WindowsLiveMessenger(wl);

				if (WebConfigSettings.TestLiveMessengerDelegation)
				{
					lnkAllowLiveMessenger.NavigateUrl = wlm.ConsentOptInUrl;
				}
				else
				{
					lnkAllowLiveMessenger.NavigateUrl = wlm.NonDelegatedSignUpUrl;
				}
	...
	private void PopulateProfileControls()
	{
		...
		if (profileConfig != null)
		{
			foreach (var propertyDefinition in profileConfig.PropertyDefinitions)
			{
				...
				if (propertyDefinition.VisibleToUser
	...
	private void UpdateUser()
	{
		...
		if (siteUser.Email != txtEmail.Text
	...
	private void FailSafeUserNameValidator_ServerValidate(object source, ServerValidateEventArgs args)
	{
		if (args.Value.Contains("<")
			|| args.Value.Contains(">")
			|| args.Value.Contains("/")
			|| args.Value.IndexOf("script", StringComparison.InvariantCultureIgnoreCase) > -1)
		{
			args.IsValid = false;
		}
	}
	...
	private void PopulateLabels()
	{
		...
		if (siteSettings.AllowUserEditorPreference && ddEditorProviders.Items.Count == 0)
		{
			...
			ddEditorProviders.Items.Insert(0, new ListItem
			{
				Value = string.Empty,
