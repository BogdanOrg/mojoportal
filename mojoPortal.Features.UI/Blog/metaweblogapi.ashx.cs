...
public class metaweblogapi : IHttpHandler
{
	...
	internal string NewPost(string blogId, string userName, string password, MWAPost sentPost, bool publish)
	{
		...
		try
		{
			...
			var post = new Blog
			{
				...
				LastModUserGuid = siteUser.UserGuid
			};
			...
			var newFriendlyUrl = new FriendlyUrl
			{
				...
				Url = newUrl,
				RealUrl = SiteUtils.GetUrlWithQueryParams("~/Blog/ViewPost.aspx", -1, pageId, post.ModuleId, post.ItemId, false)
	...
	internal string NewPage(string blogId, string userName, string password, MWAPage mwaPage, bool publish)
	{
		...
		var moduleDefinition = new ModuleDefinition(HtmlContent.FeatureGuid);
		var m = new Module
		{
			SiteId = siteSettings.SiteId,
			SiteGuid = siteSettings.SiteGuid,
			...
			CreatedByUserId = siteUser.UserId,
			ShowTitle = WebConfigSettings.ShowModuleTitlesByDefault,
			HeadElement = WebConfigSettings.ModuleTitleTag,
			ModuleOrder = 1
		};
		m.Save();
	...
	private string FormatUrl(string itemUrl, int itemId, int moduleId)
	{
		...
		else
		{
			...
			url = SiteUtils.GetUrlWithQueryParams("/Blog/ViewPost.aspx", itemId: itemId, moduleId: moduleId);
		}
	...
	private string FormatPageUrl(int pageId, string pageUrl, bool useUrl)
	{
		...
		else
		{
			...
			url = SiteUtils.GetUrlWithQueryParams("/Default.aspx", pageId: pageId);
		}
	...
	private Module GetFirstCenterPaneHtmlModule(PageSettings page)
	{
		foreach (Module m in page.Modules)
		{
			...
			{
				if (m.PaneName.IsCaseInsensitiveMatch("contentpane"))
				{
					return m;
				}
			}
	...
	private bool IsHomePage(int pageId)
	{
		...
		return (rootNode.ChildNodes[0] is mojoSiteMapNode homePage) && (homePage.PageId == pageId);
	}
